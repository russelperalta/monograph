...
export default function Header() {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            const mobileWrapper = document.getElementById('mobile-scroll-wrapper')
            const scrollTop = mobileWrapper ? mobileWrapper.scrollTop : window.scrollY
            ...
            if (scrollTimeoutRef.current) {
                // clearTimeout(scrollTimeoutRef.current) // Fixed: was explicitly calling node timeout
        ...
        // Also try to listen on mobile wrapper if it exists (check periodically or on mount)
        // A simple interval check to attach listener if it appears
        const attachInterval = setInterval(() => {
            const mobileWrapper = document.getElementById('mobile-scroll-wrapper')
            if (mobileWrapper) {
                mobileWrapper.addEventListener('scroll', handleScroll, { passive: true })
                clearInterval(attachInterval)
            }
        }, 500) // Check every 500ms for a few seconds? Or just once.
        ...
        return () => {
            ...
            const mobileWrapper = document.getElementById('mobile-scroll-wrapper')
            if (mobileWrapper) {
                mobileWrapper.removeEventListener('scroll', handleScroll)
            }
    ...
    const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, slug: string, index: number) => {
        ...
        // Scroll to section
        const windowHeight = window.innerHeight
        const targetTop = index * windowHeight
